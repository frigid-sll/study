**容器**是docker的另一个核心概念，简单来说，容器是镜像的一个运行实例所不同的是，镜像是静态的只读文件，而容器带有运行时需要的可写文件层，同时，容器中的应用进程处于运行状态。
...
这跟在本地直接执行<span style="color:red">/bin/echo 'hello world'</span>相比几乎感觉不出任何区别
